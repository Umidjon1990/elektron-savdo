...
export default function DeliveriesPage() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <SidebarNav />
      <main className="flex-1 p-4 md:p-6 pb-24 md:pb-6 overflow-auto" data-testid="deliveries-page">
        <div className="max-w-7xl mx-auto space-y-6">
          <div data-testid="deliveries-header">
            <h1 className="text-2xl font-bold text-slate-900" data-testid="text-page-title">Yetkazib berish</h1>
            <p className="text-slate-500 mt-1" data-testid="text-page-subtitle">
              Buyurtmalar yetkazib berish holati va tarixi
            </p>
          </div>
                ...
                <div className="flex gap-2">
                  {dateRanges.map((range) => (
                    <Button
                      ...
                      data-testid={`filter-date-${range.value}`}
                    >
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    {statusFilters.map((sf) => (
                      <SelectItem key={sf.value} value={sf.value} data-testid={`filter-status-${sf.value}`}>
                        {sf.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          <Card data-testid="deliveries-table">
            <CardContent className="p-0">
              {isLoading ? (
                <div className="p-8 text-center text-slate-400" data-testid="loading-state">
                  Yuklanmoqda...
                </div>
              ) : deliveries.length === 0 ? (
