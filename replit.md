...
23. **Staff Attendance System (Xodimlar nazorati)**: `staff_members` and `attendance_records` tables, Face ID verification via face-api.js (TinyFaceDetector + FaceRecognitionNet), GPS geofencing with Haversine formula, public check-in/check-out URL per staff (`/attendance/:token`), admin panel at `/store/:slug/admin/employees` with 3 tabs (Xodimlar/Davomat/Oylik), KPI cards, attendance history table
