...
26. **Supplier Management (Tovar beruvchi)**: `suppliers` table (id, tenantId, name, phone, address, note, isActive), CRUD API `/api/suppliers`, supplier dropdown with inline creation in product form, `supplierPaymentMethod` column on products (naqd/karta/nasiya), `supplierDebtStatus` (pending/partial/paid) and `supplierPaidAmount` columns for nasiya tracking, "Tovar beruvchi" 7th tab in Finance module with KPI cards (jami xarid/naqd/karta/nasiya), supplier list with expandable product details, nasiya status management (to'landi/qisman/to'lanmadi) with partial payment dialog, delete nasiya record, PieChart payment breakdown, summary API `/api/supplier-summary`, `PATCH /api/products/:id/supplier-debt` for debt status updates, supplier purchase totals linked to finance reports
