...
- **Supplier Management**: CRUD for suppliers, payment methods, and debt tracking with bulk payment options. **Debts in USD only toggle**: tenant setting `debtsInUsdOnly` (Sozlamalar → Dollar kursi) — when enabled, the "Qarz" stat card and per-supplier debt amounts on Finance → Tovar beruvchi show only USD ($) instead of so'm (UZS); the top stat card is labelled "Qarz" (renamed from "Nasiya"). **USD payment support**: in Finance → Tovar beruvchi → "To'lash", suppliers with USD-currency nasiya products see a "So'm / Dollar" currency toggle. USD payments are distributed per-product with each portion converted back to UZS using THAT product's stored `supplierCurrencyRate` (not today's rate). Full-pay snaps `supplierPaidAmount` to `costPrice * stock` to eliminate rounding dust. The cash-register expense is recorded in UZS as the sum of per-product deltas. Default dialog currency is USD only when the supplier has zero UZS-only debt remaining.
