...
25. **Courier/Delivery Management**: `isCourier` boolean on `staff_members`, `courierId` varchar on `orders` and `deliveries`, `GET /api/couriers` endpoint, Kuriyerlar tab (courier list with KPI cards), Yetkazish tab (delivery tracking with courier dropdown assignment, status updates, date/status/courier filters), courier dropdown in orders-tab replacing text input, server-side courierId validation
