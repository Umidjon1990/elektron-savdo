...
26. **Supplier Management (Tovar beruvchi)**: `suppliers` table (id, tenantId, name, phone, address, note, isActive), CRUD API `/api/suppliers`, supplier dropdown with inline creation in product form, `supplierPaymentMethod` column on products (naqd/karta/nasiya), "Tovar beruvchi" 7th tab in Finance module with KPI cards (jami xarid/naqd/karta/nasiya), supplier list with expandable product details, PieChart payment breakdown, summary API `/api/supplier-summary`, supplier purchase totals linked to finance reports
